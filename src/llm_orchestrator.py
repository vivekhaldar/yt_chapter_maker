...
class LLMOrchestrator:
        ...
        while retries < self.max_retries:
            try:
                ...
                if retries == self.max_retries:
                    raise ChapterMakerError(f"API call failed after {self.max_retries} retries: {str(e)}")
                logger.warning(f"API call failed, retrying in {self.retry_delay} seconds...")
    ...
    Each timestamp should be on a new line. Do not include any other text, explanations, or formatting in your response."""
        messages = [
            {"role": "system", "content": "You are a helpful assistant that creates YouTube chapters from transcripts."},
    ...
    Third Title Here"""
        messages = [
            {"role": "system", "content": "You are a helpful assistant that creates engaging but not clickbait YouTube titles."},
            {"role": "user", "content": f"{prompt}\n\nHere's the transcript:\n\n{srt_content}"}
        ]
